...
import secrets
import logging
from odoo.addons.az_lead_extract_auth.controllers import constants
...
class ApiAccessToken(models.Model):
    ...
    def create_token(self, user):
        ...
        token_expiration = int(self.env['ir.config_parameter'].sudo().get_param('az_lead_extract_auth.api_token_expiration')) or 0
    ...
    def check_access_token(self, token):
        ...
        if not token:
            token = False
            msg = constants.MSG_INVLID_TOKEN
        elif token.expiry_date and token.expiry_date < datetime.now():
            token.unlink()
            token = False
            msg = constants.MSG_TOEKN_EXPIRED
