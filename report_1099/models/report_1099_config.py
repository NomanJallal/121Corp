...
class Report1099Config(models.Model):
    ...
    is_1099 = fields.Boolean(tracking=True, default=True)
    ssn_ein = fields.Selection(
        [("ssn", "SSN"), ("ein", "EIN")], string="SSN/EIN No.", tracking=True
    )
    ssn_number = fields.Char("SSN Number", help="Format : XXX-XX-XXXX", tracking=True)
    ein_number = fields.Char("EIN Number", help="Format : XX-XXXXXXX", tracking=True)
    ...
    _sql_constraints = [
        ("default_code_partner_id", "unique (partner_id)", "Partner should be unique!")
    ]
        ...
        for rec in self:
            if rec.ssn_ein == "ssn" and rec.ssn_number:
                ...
                if len(rec.ssn_number) != 11:
                    is_wrng_ssn = True
                if len(rec.ssn_number) == 11:
                    result = re.match(r"[0-9]{3}-[0-9]{2}-[0-9]{4}", rec.ssn_number)
                    if not result:
                        is_wrng_ssn = True
                if is_wrng_ssn:
